	...
	m_bSkeletonPrjInitialized(false)
{
	...
	addOption('t', true, "task-list", "a file containing a list of "
			"visualization tasks");
	addOption('i', true, "FA-indices", "a file containing tube and vertex index "
			" of FA points");
...
int CSitubeRender::mainstay() 
{
	...
	if ( 0 != _selectFApoints(20.0, 15.0f) ) {
		cerr << "FATAL: failed to load FA point indices.\n";
		return -1;
	}
...
void CSitubeRender::draw() 
{
	...
	*/
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		if ( m_boxes.size() > 0 ) {
			if ( m_bOR ) {
			   if ( m_bRemovalbased == isTubeInSelbox(m_alltubevertices[idx]) ) {
				   /*
		...
		glBegin(GL_LINES);
			...
			glVertex3f( m_v1e.x, m_v1e.y, m_v1e.z );

			glVertex3f( m_v2.x, m_v2.y, m_v2.z );
			glVertex3f( m_v2e.x, m_v2e.y, m_v2e.z );
		glEnd();
		printText("1", m_v1e.x, m_v1e.y, m_v1e.z, 
				1.0, 1.0, 0.0, 0.6f, GLUT_BITMAP_TIMES_ROMAN_24);
		printText("2", m_v2e.x, m_v2e.y, m_v2e.z, 
				1.0, 1.0, 0.0, 0.6f, GLUT_BITMAP_TIMES_ROMAN_24);
		glEnable(GL_LIGHTING);
	}
...
void CSitubeRender::drawSkeleton()
{
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		...
		if ( m_boxes.size() > 0 ) {
			//if ( m_bOR ) 
			{
			   if ( m_bRemovalbased == isTubeInSelbox(line) ) {
				   continue;
			   }
			}
			/*
			...
			if ( "" != m_strfntask ) {
				if ( m_taskbox.iscovered()/* && !m_taskbox.isPrompt()*/ ) {
					...
					if ( m_taskbox.next() ) {
						...
						std::transform(m_strKey.begin(), m_strKey.end(),
								m_strKey.begin(), ::toupper);
						std::transform(m_strAnswer.begin(), m_strAnswer.end(),
								m_strAnswer.begin(), ::toupper);
						if ( m_strKey == m_strAnswer ) {
							m_strAnswer += " (correct).";
						}
						else {
							m_strAnswer += " (wrong).";
...
void CSitubeRender::_findNearest(const _T& x0, const _T& y0, const _T& z0,
								_T& x1, _T& y1, _T& z1,
								unsigned long& idxl,
								unsigned long& idxp)
{
	_T mind = 0xffffffffff, curd;
	_T x,y,z;
	unsigned long szTotal = (unsigned long)m_alltubevertices.size();
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		const vector<GLfloat> & tube = m_alltubevertices[ idx ];
		unsigned long szPts = static_cast<unsigned long>( tube.size()/3 );
		for (unsigned long idx1 = 0; idx1 < szPts; idx1++) {
			x = tube [ idx1*3 + 0 ], 
			y = tube [ idx1*3 + 1 ], 
			z = tube [ idx1*3 + 2 ];

			curd = magnitude(x-x0, y-y0, z-z0);
			if ( curd < mind ) {
				x1 = x, y1 = y, z1 = z;
				idxl = idx;
				idxp = idx1;
...
int CSitubeRender::_selectFApoints(GLfloat dist, GLfloat scale, bool bAlongNormal)
{
	unsigned long idxl1, idxp1, idxl2, idxp2;

	// try to load predefined points firstly
	if ( "" != m_strfnFApoints ) {
		ifstream ifs(m_strfnFApoints.c_str());
		if ( !ifs.is_open() ) {
			return -1;
		}

		// we expect only three lines in this file
		// 1st line: the task key telling which point, among A and B, is of the
		// higher FA value
		ifs >> m_strKey;
		// 2nd line: line and vertex index of the first FA point
		ifs >> idxl1 >> idxp1;
		// 3rd line: line and vertex index of the first FA point
		ifs >> idxl2 >> idxp2;
		ifs.close();

		m_v1.update(
				m_alltubevertices[idxl1][idxp1*3+0],
				m_alltubevertices[idxl1][idxp1*3+1],
				m_alltubevertices[idxl1][idxp1*3+2]);
		m_v2.update(
				m_alltubevertices[idxl2][idxp2*3+0],
				m_alltubevertices[idxl2][idxp2*3+1],
				m_alltubevertices[idxl2][idxp2*3+2]);
	}
	else {
		_findNearest<GLfloat>(
				(m_maxCoord[0]+m_minCoord[0])/2-dist/2.0, 
				(m_maxCoord[1]+m_minCoord[1])/2.15,
				(m_maxCoord[2]+m_minCoord[2])/2,
				m_v1.x, m_v1.y, m_v1.z, idxl1, idxp1);

		_findNearest<GLfloat>(
				(m_maxCoord[0]+m_minCoord[0])/2+dist/2.0,
				(m_maxCoord[1]+m_minCoord[1])/2.15,
				(m_maxCoord[2]+m_minCoord[2])/2.0,
				m_v2.x, m_v2.y, m_v2.z, idxl2, idxp2);
	}

	/*
	for (size_t i = 0; i < m_edgeflags.size(); i++) {
		if ( m_edgeflags[i] == GL_TRUE ) {
			cout << "currently selected fiber No. = " << i << "\n";
		}
	}
	*/

	if ( bAlongNormal ) {
		m_v1e = m_v1 + _point_t<GLfloat>(
				m_alltubenormals[idxl1][idxp1*3+0],
				m_alltubenormals[idxl1][idxp1*3+1],
				m_alltubenormals[idxl1][idxp1*3+2])*scale;

		m_v2e = m_v2 + _point_t<GLfloat>(
				m_alltubenormals[idxl2][idxp2*3+0],
				m_alltubenormals[idxl2][idxp2*3+1],
				m_alltubenormals[idxl2][idxp2*3+2])*scale;
	}
	else {
		m_v1e = m_v1 + _point_t<GLfloat>(-1,0,0)*scale;

		m_v2e = m_v2 + _point_t<GLfloat>(1,0,0)*scale;
	}

	// make a consistent difference between these two selected points
	/*
	m_alltubecolors[idxl1][idxp1*3+0] = .5;
	m_alltubecolors[idxl2][idxp2*3+0] = 1.0;
	m_alltubecolors[idxl2][idxp2*3+1] = m_alltubecolors[idxl1][idxp1*3+1];
	m_alltubecolors[idxl2][idxp2*3+2] = m_alltubecolors[idxl1][idxp1*3+2];
	*/
	return 0;
}
