...
{
	// add default options
	addOption('h', false, "help", "show this usage assistant information");
	addOption('v', false, "version", "giving version info, not definitely"
		   " however, as the identity of different versions of this program");
	addOption('V', false, "verbose", "switch on the debug mode so that to look"
			" what the program does in detail");
...
void CApplication::show_helpmsg(bool detailed)
{
	...
	for (; it != m_optinfo.end(); ++it ) {
		...
		// option
		for (int icnt = 0; icnt < nTotalLen / nLen + 1; ++icnt) {
			fprintf(stdout, "\t   ");
			if ( it->second.optname.length() >= 2) {
			...
			int segLen = min(nLen, nsLen - startPos);
			fprintf(stdout, "%s\n", 
					it->second.message.substr( startPos, segLen ).c_str());
			nPos += nLen;
		}
	}
