...
{
	unsigned long szFibers=0, i=0, curIdx=0, mid;
	...
	while ( ifs && i < szFibers ) {
		...
		mid = m_alltubevertices[curIdx].size()/3 / 2;
		for (int j = 0; j < 3; j++) {
			if ( mins[j] > m_alltubevertices[curIdx][mid*3+j] ) {
				mins[j] = m_alltubevertices[curIdx][mid*3+j];
			}

			if ( maxs[j] < m_alltubevertices[curIdx][mid*3+j] ) {
				maxs[j] = m_alltubevertices[curIdx][mid*3+j];
			}
		}
	...
	// fix the size of the sole box to make it a slightly larger that is needed
	// for covering all the start points of the loaded fibers
	GLfloat ds = sqrt(m_endSize*m_endSize/3.0);
...
bool CSitubeRender::_checkAnswer()
{
	...
	// retrieve fibers in currently "marked" bundle, i.e. the region wrapped
	// around by the box (NOTE here the box lost its culling function already.)
	
	unsigned long szTotal = m_loader.getSize(), nIn = 0;
	m_bIboxCulling = true;
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		if ( !isLineInBox<GLfloat>( 
					&m_alltubevertices[idx][0], 
					m_alltubevertices[idx].size(),
					6, 3, 
					-(GLfloat)( m_minCoord[0] + m_maxCoord[0] )/2,
		...
		if ( m_expectedFiberIndices.end() != 
				m_expectedFiberIndices.find(idx) ) {
			nIn ++;
		}
	}
	m_bIboxCulling = false;
