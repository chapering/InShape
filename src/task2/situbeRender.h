...
// @Mar. 16th
//			--- fix the box size to the bounding size of the target fiber bundle
//			loaded as the task key;  and stretching is proscribed therefore for
//			the box. 
//			NOTE: since _loadFiberIdx will call m_boxes[0].setMinMax(), so it
//			must go after addBox() in mainstay(). Otherwise, there will be a
//			run-time fatality - segfault.
//
...
private:
	...
	// the dimension of the expectedly selected fiber bundle end
	GLfloat m_endSize;
...
private:
	...
	// load the expectedly selected fibers by indices 
	int _loadFiberIdx();
	// compute the rate of correct marking thus give result as the evaluation
	// of user's task performance
	bool _checkAnswer();
};
