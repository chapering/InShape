	...
	m_strfnTumorBox("")
{
	...
	addOption('t', true, "task-list", "a file containing a list of "
			"visualization tasks");
	addOption('u', true, "tumor-file-name", "the name of file containing "
			"geometry of the accessory object in the format of sm");
	addOption('i', true, "tumor-box-info", "name of file holding tumor bounding "
			"box information, preceded by the task answer.");
...
int CSitubeRender::loadGeometry() 
{
	...
	// load accessory object if applicable
	if ( "" != m_strfntumor  &&
		0 != m_tumorLoader.load(m_strfntumor) ) {
		m_cout << "Loading auxiliary geometry failed - aborted abnormally.\n";
		return -1;
	}
...
int CSitubeRender::mainstay() 
{
	...
	// load the tumor bounding box information, along with the task key (for
	// task 5 only)
	if ( "" == m_strfnTumorBox || 0 != _loadTumorBoxpos() ) {
...
void CSitubeRender::draw() 
{
	...
	*/
	...
	// draw the accessory tumor object if applicable
	if ( "" != m_strfntumor ) {
		GLdouble offset = 0, scaleFactor = 1;
		GLdouble tumor_Xmin =  m_tumorLoader.getMinCoord(0)*
				(m_tumorBoxMax.x - m_tumorBoxMin.x)/
				(m_tumorLoader.getMaxCoord(0) - m_tumorLoader.getMinCoord(0))/scaleFactor,
				tumor_Ymin = m_tumorLoader.getMinCoord(1) *
				(m_tumorBoxMax.y - m_tumorBoxMin.y)/
				(m_tumorLoader.getMaxCoord(1) - m_tumorLoader.getMinCoord(1))/scaleFactor,
				tumor_Zmin = m_tumorLoader.getMinCoord(2) *
				(m_tumorBoxMax.z - m_tumorBoxMin.z)/
				(m_tumorLoader.getMaxCoord(2) - m_tumorLoader.getMinCoord(2))/scaleFactor;

		glTranslatef( ( m_minCoord[0] + m_maxCoord[0] )/2,
			( m_minCoord[1] + m_maxCoord[1] )/2,
			( m_minCoord[2] + m_maxCoord[2] )/2);
		// place the tumbor object exactly into the loaded bounding box
		glTranslatef(offset + m_tumorBoxMin.x - tumor_Xmin,
				offset + m_tumorBoxMin.y - tumor_Ymin,
				offset + m_tumorBoxMin.z - tumor_Zmin);

		// scale the tumor objec coordinate as such its bounding box will be exactly the same 
		// as the loaded tumor bounding box
		glScalef( 
				(m_tumorBoxMax.x - m_tumorBoxMin.x)/
				(m_tumorLoader.getMaxCoord(0) - m_tumorLoader.getMinCoord(0))/scaleFactor,
				(m_tumorBoxMax.y - m_tumorBoxMin.y)/
		...
		glVertexPointer(3, GL_DOUBLE, 0, &m_tumorLoader.m_alltubevertices[0]);
		glNormalPointer(GL_DOUBLE, 0 ,&m_tumorLoader.m_alltubenormals[0]);
		glDrawElements(GL_TRIANGLES, m_tumorLoader.m_alltubefaceIdxs.size(), 
				GL_UNSIGNED_INT, &m_tumorLoader.m_alltubefaceIdxs[0]);
		glPopClientAttrib();
	}
...
{
	ifstream ifs(m_strfnTumorBox.c_str());
	if ( !ifs.is_open() ) {
		return -1;
	}

	// we only care about the 3 lines of the file
	// 1st line: the task key
	if ( !ifs ) return -1;
	ifs >> m_strKey;
	...
	ifs >> m_tumorBoxMin.x >> m_tumorBoxMin.y >> m_tumorBoxMin.z;

	// 3rd line: the coordinate of the major corner of the tumor bounding box
	if ( !ifs ) return -1;
	ifs >> m_tumorBoxMax.x >> m_tumorBoxMax.y >> m_tumorBoxMax.z;

	cout << "loaded:";
	cout << m_tumorBoxMax << m_tumorBoxMin << "\n";
	return 0;
}
