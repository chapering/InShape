...
//		2. Load an extraneous object as a symbolic tumor, this is useful in
//		certain visualization study, two types of placement needed:
//		- the tumor object is tangential to the primary object;
//		- the tumor object is slightly outlying from the primary object,
//		ideal distance is about the tube interval
// @Mar. 16th
//		. For task5, to locate the tumor potato desirably, load  boudning box
//		information(coordinates of the two opposite corners) from another input
//		file which is manually sampled before. Then the tumor will be placed as
//		such that potato is exactly bounded by the imaginary box.
//		. since now if the tumor object will be placed tangential to / slightly 
//		deviating from the primary object is implicitly told by the tumor
//		bounding box information loaded from m_strfnTumorBox, m_bTumorTangential
//		is not need anymore, thus obviated.
//
...
private:
	...
	/* for accessory object, a symbolic tumor for instance, loading */
	CSmLoader m_tumorLoader;

	/* source file holding the most essential streamline geometry, in tgdata
	 * format */
	string m_strfnsrc;
	...
	string m_strfntask;
	/* source file holding accessory object geometry, in sm format*/
	string m_strfntumor;
	...
	// tumor bounding box information, prefixed with the task key, in a file
	// whose name is retrieved from the command line, there are only
	// a line of string as task key, plus 2 lines of 3 floating numbers each in
	// the file
	std::string m_strfnTumorBox;

	// the opposite corner of the tumor bounding box
	_point_t<GLfloat> m_tumorBoxMin, m_tumorBoxMax;
