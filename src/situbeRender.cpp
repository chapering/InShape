...
void CSitubeRender::buildTubefromLine(unsigned long lineIdx) 
{
	...
	for (unsigned long idx1 = 0, idx2 = 1; idx1 < szPts; idx1++) {
		...
		if ( m_expectedFiberIndices.find( lineIdx ) != m_expectedFiberIndices.end() ) {
			r = g = 1.0;
			b = .0;
		}
		...
		for (GLubyte l = 0; l < m_lod; ++l) {
			...
			tube_ctxcolors [ idx1*4 * m_lod + 4*l + 3 ] = 0.5;
...
void CSitubeRender::glInit(void) 
{
	...
	glLightfv(GL_LIGHT0, GL_LINEAR_ATTENUATION, &d);
	glLightfv(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, &d);
...
int CSitubeRender::dumpBoxpos(int bidx, const char* fnBoxpos)
{
	if ( m_boxes.size() < 1 ) {
		return -1;
	}
...
void CSitubeRender::drawSkeleton()
{
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		...
		if ( m_boxes.size() > 0 ) {
			...
			if ( m_bOR ) 
			{
			   if ( isTubeInSelbox(line) ) {
				   continue;
			   }
			}
			*/
			if ( m_bBrushEffect && !m_skeletonEdgeflags[idx] ) {
				continue;
			}
...
int CSitubeRender::_loadFiberIdx()
{
	...
	GLfloat mins[3] = {0xffff, 0xffff, 0xffff}, 
			maxs[3] = {-0xffff, -0xffff, -0xffff};
