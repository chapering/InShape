...
int CSitubeRender::serializeTubes(const string& fnobj) 
{
	...
	unsigned long szVertex, szNormal, szFace, fidxBase = 1, nidxBase = 1;
	...
	for (unsigned long tIdx = 0; tIdx < szTubes; tIdx++) {
		...
		ofs << "o Tube_" << tIdx << endl;
		ofs << "g Tube_" << tIdx << endl;
		...
		for (unsigned long fIdx = 0; fIdx < szFace; fIdx ++) {
			...
			for (int pIdx = 0; pIdx < 4; pIdx++) {
				...
				ofs << m_alltubefaceIdxs[ tIdx ][ fIdx*4 + pIdx ] + fidxBase << "//"
					<< m_alltubefaceIdxs[ tIdx ][ fIdx*4 + 0 ] + nidxBase;
				if ( pIdx < 3 ) {
					ofs << " ";
				}
			}
