...
CSitubeRender::CSitubeRender(int argc, char **argv) : CGLIBoxApp(argc, argv),
	...
	m_fRadius(0.25),
	m_fbdRadius(20.0),
	m_fMaxSegLen(-0xffffffffff),
	m_fMinSegLen(0xffffffffff),
	m_colorschemeIdx(CLSCHM_ANATOMY_HEURISTIC),
	...
	m_bSuspended(false),
	m_strfnFiberIdx(""),
	m_endSize(0xffffffffff)
{
	for (int i=0;i<3;++i) {
		m_maxCoord[i] = -0xffffffffff;
		m_minCoord[i] = 0xffffffffff;
	}
...
void CSitubeRender::mouseResponse(int button, int state, int x, int y)
{
	...
	*/
	if ( GLUT_LEFT_BUTTON == button ) {
		m_bMouseOnlyForBox = GLUT_UP == state;
...
int CSitubeRender::_loadFiberIdx()
{
	...
	GLfloat mins[3] = {0xffffffffff, 0xffffffffff, 0xffffffffff}, 
			maxs[3] = {-0xffffffffff, -0xffffffffff, -0xffffffffff};
	...
	m_endSize = _point_t<GLfloat> (mins[0],mins[1],mins[2]).distanceTo(
				_point_t<GLfloat> (maxs[0],maxs[1],maxs[2]) );

	// fix the size of the sole box to make it a slightly larger than is needed
	// for covering all the start points of the loaded fibers
	GLfloat ds = m_endSize;//sqrt(m_endSize*m_endSize/3.0);
	m_boxes[0].setMinMax(m_minCoord[0], m_minCoord[1], m_minCoord[2],
			m_minCoord[0] + ds, m_minCoord[1] + ds, m_minCoord[2] + ds);

	return 0;
}

bool CSitubeRender::_checkAnswer()
{
	// we attack against the gross brute-force behavior : we need an acceptably
	// tightly bound marking
	GLfloat curBoxSize = (GLfloat)m_boxes[0].getDimension();

	// Threshold 1 : bounding size ratio default as <= 2
	GLfloat tightness = curBoxSize / m_endSize;
	if ( tightness  >  2.0 ) {
		m_cout << "Answer : marking incorrectly [ too roughly bounding - "
			"bounding tightness = " << tightness << " ].\n";
		return false;
	}

	// calculate number of fibers whose end point falls inside the currently
	// placed box and the answer will be given by the ratio of this number to
	// the total of the loaded reference fibers
	
	unsigned long nIn = 0, idx, last;
	GLfloat dx = -(GLfloat)( m_minCoord[0] + m_maxCoord[0] )/2,
			dy = -(GLfloat)( m_minCoord[1] + m_maxCoord[1] )/2,
			dz = -(GLfloat)( m_minCoord[2] + m_maxCoord[2] )/2;

	std::set<unsigned long>::iterator it = m_expectedFiberIndices.begin();
	while ( it != m_expectedFiberIndices.end() ) {
		idx = *it; 
		last = m_alltubevertices[idx].size()/3 - 1;
		if ( m_boxes[0].isInside( 
					m_alltubevertices[idx][last*3+0] + dx,
					m_alltubevertices[idx][last*3+1] + dy,
					m_alltubevertices[idx][last*3+2] + dz) ){
			nIn ++;
		}
		it ++;
	}

	// Threshold2 : correct fiber coverage default as >= 0.80
	GLfloat coverage = nIn * 1.0 / m_expectedFiberIndices.size();
	if (  coverage < 0.80 ) {
		m_cout << "Answer : marking incorrectly [ insufficient fiber coverage - "
			<< (coverage*100) << "% ].\n";
		return false;
	}

	m_cout << "Answer : Marking correctly [ bounding tightness : " <<
		tightness << ", fiber coverage : " << coverage << " ].\n";
	return true;
}
...
void CSitubeRender::_drawMarkers()
{
	...
	while ( it != m_expectedFiberIndices.end() ) {
		curIdx = *it; 
		x = m_alltubevertices[curIdx][0*3+0];
		y = m_alltubevertices[curIdx][0*3+1];
		z = m_alltubevertices[curIdx][0*3+2];
