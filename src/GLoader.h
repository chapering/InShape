...
int CGLoader<_T>::memmap()
{
	...
	m_fsize = sb.st_size;
#else
   HANDLE hFile = CreateFile(m_fn.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
                     OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

   if (hFile == INVALID_HANDLE_VALUE){ 
      cerr << "FATAL: Open file failed - " << m_fn << "[ with error: " 
			<<  GetLastError() << " ]" << endl;
      CloseHandle(hFile);
	  return -1;
   }
   ...
   ostringstream memName;
   /*
   string fnalone;
   size_t spos = m_fn.rfind("\\"), epos = m_fn.rfind(".");
   if ( spos != string::npos ) {
      fnalone.assign( m_fn.begin() + spos + 1, 
                      epos==string::npos?m_fn.end(): m_fn.begin()+epos);
   }
   else {
      fnalone = m_fn;
   }
   */
   memName << "Local\\mem" << time(NULL);
   string ab = memName.str();
   HANDLE hMapFile = CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, fsize, ab.c_str());
         
   char* pData = (char*) MapViewOfFile(hMapFile, FILE_MAP_READ, 0, 0, fsize);
 
   if (pData == NULL){ 
      cerr << "FATAL: mmap failed [ with error: " << GetLastError() << " ]" 
			<< endl;
...
#else
    int ret = UnmapViewOfFile(m_pdata);
#endif	
	if (-1 == ret) {
