...
int COptionPanel::_calcBtnLayout()
{
	...
	for (int i = 0; i <= numRow; ++i) {
		...
		for (int j = 0; j < numPerRow; ++j) {
			currtx = curlbx + btnW*signx - (PBL_EAST==m_layout?spacing:0);
			...
			(m_buttons)[ ibtn ].textpos.update( 
					PBL_EAST==m_layout?curlbx + spacing + (btnW - curW)/3.0 : curlbx+curW/3.0,
					curlby+spacing, __z );
		...
		curlby -= (btnH + (PBL_EAST==m_layout?3:1)*spacing)*signy;
	}
