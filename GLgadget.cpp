...
void CGLGadget::display()
{
	...
	if ( !m_bStereo ) {
		...
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glFrustum(m_stereoinfo.left, m_stereoinfo.right, -m_stereoinfo.asp, m_stereoinfo.asp, mindist/15.0, maxdist*4.0);

		glMatrixMode(GL_MODELVIEW);

		glPushMatrix();
		glLoadIdentity();
		gluLookAt(0, 0, m_dz*5.0, 0, 0, 0, 0, 1, 0);
		...
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glFrustum(-m_stereoinfo.right, -m_stereoinfo.left, -m_stereoinfo.asp, m_stereoinfo.asp, mindist/15.0, maxdist*4.0);
...
void CAnatomyAxis::draw()
{
	...
	COLORING(d, .0, .0);
	_drawArrow(point_t(0,0,0), point_t(d, 0, 0), 3);
	printTextIn3D("R", d + labelOffset, 0, 0, r, g, b);

	// arrow at the end of left-axis 
	//glColor4fv(m_colors[0]);
	COLORING(-d, .0, .0);
	_drawArrow(point_t(0,0,0), point_t(-d, 0, 0), 3);
	printTextIn3D("L", -(d + labelOffset), 0, 0, r, g, b);

	// arrow at the end of superior-axis 
	//glColor4fv(m_colors[1]);
	COLORING(.0, d, .0);
	_drawArrow(point_t(0,0,0), point_t(0, d, 0), 3);
	printTextIn3D("A", 0, d + labelOffset, 0, r, g, b);

	// arrow at the end of inferior-axis 
	//glColor4fv(m_colors[1]);
	COLORING(.0, -d, .0);
	_drawArrow(point_t(0,0,0), point_t(0, -d, 0), 3);
	printTextIn3D("P", 0, -(d + labelOffset), 0, r, g, b);

	// arrow at the end of anterior-axis 
	//glColor4fv(m_colors[2]);
	COLORING(.0, .0, d);
	_drawArrow(point_t(0,0,0), point_t(0, 0, d), 3);
	printTextIn3D("S", 0, 0, d + labelOffset, r, g, b);

	// arrow at the end of anterior-axis 
	//glColor4fv(m_colors[2]);
	COLORING(.0, .0, -d);
	_drawArrow(point_t(0,0,0), point_t(0, 0, -d), 3);
	printTextIn3D("I", 0, 0, -(d + labelOffset), r, g, b);
}
...
int CRectColorMap::_calcRectGeometry()
{
	...
	char mark[64];
	mark [ sprintf(mark, "%.2f", m_tickmax) ] = 0;
...
void COptionPanel::draw()
{
	...
	if (0 == _calcBtnLayout()) {
		for (size_t i = 0; i < szBtn; i++) {
			(m_buttons)[i].draw();
			//cout << "button: " << m_buttons[i].text << " drawn.\n";
		}
	}
}
...
int COptionPanel::setLayout(int layout)
{
	m_layout = layout;
	return 0;
}
...
{
	int signx = 1, signy = 1;
	GLdouble startx = 0, starty = 0, twidth, theight, __z = 0;
	...
	for (int i=0; i < szbtn; i++) {
		if ( PBL_EAST == m_layout ) {
			curW = m_buttons[i].text.length()*3;
		}
		else {
			curW = glutBitmapLength(GLUT_BITMAP_TIMES_ROMAN_10, 
					(const unsigned char*)m_buttons[i].text.c_str());
		}
		if ( curW > maxW ) {
			maxW = curW;
		}
	...
	int btnW = 6+spacing, btnH = 4+spacing;
		...
		case PBL_SOUTH:
			{
				...
				starty = -d/6.0+btnH;
				numPerRow = fabs(twidth) / (spacing + btnW);
			}
			break;
		case PBL_EAST:
			{
	...
	for (int i = 0; i <= numRow; ++i) {
		...
		for (int j = 0; j < numPerRow; ++j) {
			currtx = curlbx + btnW*signx - (PBL_EAST==m_layout?spacing*4:0);
			...
			if ( PBL_EAST == m_layout ) {
				curW = m_buttons[ibtn].text.length()*6;
			}
			else {
				curW = glutBitmapLength(GLUT_BITMAP_TIMES_ROMAN_10, 
						(const unsigned char*)m_buttons[i].text.c_str());
			}
			(m_buttons)[ ibtn ].textpos.update( 
					//PBL_EAST==m_layout?(curlbx + spacing/2.0 + (btnW - curW)/4.0) : (curlbx+curW/3.0),
					PBL_EAST==m_layout?(curlbx + spacing) : (curlbx+curW/3.0),
					curlby+spacing, __z );
		...
		curlby -= (btnH + (PBL_EAST==m_layout?1:1)*spacing)*signy;
	}
