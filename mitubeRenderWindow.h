...
// @June 14th
//		.Change parent class to CSitubeRenderMult to support automatic
//		geometrical model calibration in order to enhance the interaction
//		synchronization
//		.m_bCalibrate is added as Calibration controling switch
// @July 21th
//		.dispatch "-i" for indices of fiber to be marked to each render since a
//		single render will not be assigned to multiple "-i" options (it is the
//		opposite case for "-j" to indicate multiple box to load). 
//
...
class CMitubeRenderWindow: public CSitubeRenderMult {
public:
...
private:
	...
	// selection boxes, etc.
	vector<CSitubeRenderMult*> m_renders;
	...
	// if apply automatic geometrical calibration
	bool m_bCalibrate;
