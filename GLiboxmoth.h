...
protected:
	...
	int addSphere(const CIselsphere& selphere);
	...
	template <typename _T>
	bool isLineInSphere(const _T* pline, GLsizei size, 
					GLsizei unit = 3, GLsizei offset = 0,
					_T dx = 0,  _T dy = 0, _T dz = 0) {
		// sphere should contain at least one point on the line
		if ( !pline || unit <= 0 || size/unit <= 0 ) {
			return false;
		}

		for (GLsizei j = 0; j < size/unit; j++) {
			if (m_spheres[0].isInside(pline[j*unit+offset+0] + dx, 
						pline[j*unit+offset+1] + dy, 
						pline[j*unit+offset+2] + dz)) {
				return true;
			}
...
protected:
	...
	/* record currently selected sphere */
	int m_curselsphereidx;
	/* record currently highlighte sphere */
	int m_curhlsphereidx;
