...
int CSitubeRender::mainstay() 
{
	...
	//addButton("c-: no contex", "bicons/6.bmp","c+: show context"); // show context or no context
#ifdef DWI_EMBEDDING
	addButton("d", "bicons/7.bmp","x"); 
	addButton(">"); 
	addButton("<"); 
#endif
	//addButton("r: switch to removing mode", "bicons/6.bmp","b: switch to brushing mode"); // brushing or non-brushing

	m_nselbox = 1;//m_strfnTumorBoxes.size();
...
{
	switch (event) {
		case 4: // show context or not
			m_bShowContext = !m_bShowContext;
			m_buttons[event].switchStatus();
			break;
#ifdef DWI_EMBEDDING
		case 5:
