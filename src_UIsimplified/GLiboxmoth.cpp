	...
	m_bRightmenuAvailable(false)
{
	...
	//addButton("+: clone this box", "bicons/1.bmp");
	addButton("OR ", "bicons/3.bmp","AND"); // AND or OR
	//addButton("e: switch to removal mode", "bicons/4.bmp","a: switch to selection mode"); // exclusive or active selection mode
	//addButton("-: delete this box", "bicons/2.bmp");
}
...
void CGLIBoxApp::_createRCMenu()
{
	...
	for (size_t idx = 0; idx < szbtn; idx++) {
		string curtext = m_buttons[idx].isPrimary?m_buttons[idx].text:
						m_buttons[idx].text2;
		if ( curBox != -1 && ('a' == m_buttons[idx].text[0] || 'e' == m_buttons[idx].text[0] ) ) {
			curtext = m_boxes[curBox].isRemovalBased()?"a: switch to selection mode":"e: switch to removal mode";
		}
