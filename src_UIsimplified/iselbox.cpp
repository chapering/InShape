...
void _button_t::draw()
{
	...
	if ( !curpIconImg ) { // no icon assigned, use text instead
		...
		if (isPressed) {
			...
			printText(curText.c_str(), 
					textpos.x, textpos.y, textpos.z,
					1, 1, 0, 1.0, GLUT_BITMAP_TIMES_ROMAN_24);
		}
		...
		if (0 == _calcBtnLayout()) {
			for (size_t i = 0; i < szBtn; i++) {
				...
				if ("a" == (*m_pbuttons)[i].text ||
					"e" == (*m_pbuttons)[i].text ) {
					(*m_pbuttons)[i].text = (m_bRemovalbased?"e":"a");
				}
				(*m_pbuttons)[i].draw();
			}
		}
...
int CIselbox::_calcBtnLayout(int fidx)
{
	...
	GLdouble mvmat[16];
	glGetDoublev(GL_MODELVIEW_MATRIX, mvmat);
	GLdouble sf = 1;//fabs(mvmat[14]/(m_dz*3))/2.5;
	int btnW = 4+2, btnH = 3+2;
	btnW *= sf;
	btnH *= sf;
