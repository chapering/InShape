// ----------------------------------------------------------------------------
// mitubeRenderView : a multiple-view itubeRender capsule, loading multiple
	...
	m_bCalibrate(false)
{
	...
	addOption('c', false, "calibrate", "application of automatic geometrical calibration");
}
...
int CMitubeRenderView::handleOptions(int optv) 
{
	switch( optv ) {
		...
		case 'c':
			...
			return 0;
		default:
			return CSitubeRenderMult::handleOptions( optv );
	}
	return 1;
}
...
int CMitubeRenderView::_createAllRenders()
{
	...
	for (int i = 0; i < nRender; i++) {
		...
		m_renders.push_back( new CSitubeRenderMult( nItmes, m_allargvs[ i ] ) );
	}
...
int CMitubeRenderView::_armRenders() 
{
	...
	m_renders[0]->run();
	point_t minpt( m_renders[0]->m_loader.getMinCoord(0),
			m_renders[0]->m_loader.getMinCoord(1),
			m_renders[0]->m_loader.getMinCoord(2) );
	point_t maxpt( m_renders[0]->m_loader.getMaxCoord(0),
			m_renders[0]->m_loader.getMaxCoord(1),
			m_renders[0]->m_loader.getMaxCoord(2) ) ;

	for (int i = 1; i < nRender; i++) {
		m_renders[i]->run();

		if ( m_bCalibrate ) {
			m_renders[i]->_calibrateLineGeometry( minpt, maxpt );
		}
	}

	m_renders[0]->run();
	point_t mintubept( m_renders[0]->m_minCoord[0],
				m_renders[0]->m_minCoord[1],
				m_renders[0]->m_minCoord[2] );
	point_t maxtubept( m_renders[0]->m_maxCoord[0],
				m_renders[0]->m_maxCoord[1],
				m_renders[0]->m_maxCoord[2] );

	for (int i = 1; i < nRender; i++) {
		m_renders[i]->run();
		if ( m_bCalibrate ) {
			m_renders[i]->_calibrateTubeGeometry( mintubept, maxtubept );
		}
	}
