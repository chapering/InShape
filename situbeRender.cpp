	...
	*/
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		if ( m_boxes.size() > 0 ) {
			/*
			if ( m_bOR ) {
			   if ( !isTubeInSelbox(m_alltubevertices[idx]) ) {
			...
			*/
			if ( m_bBrushEffect && !m_edgeflags[idx] ) {
				continue;
			}
			if ( !isLineInBox<GLfloat>( 
					...
					m_bOR ) ) {
				/*
				 * flags of streamlines that will disappear are now set as GL_FALSE, 
