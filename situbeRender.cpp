...
GLdouble heinFunc(GLdouble x, GLdouble n)
{
	return  pow(1.0 - pow( fabs( x/g_a ), n ), 1.0/n) * g_b;
}

void SuperEllipseShow(int w, int h) 
{
	...
	GLdouble d = 10.0;
	if (w <= h)
		glOrtho(-d, d, -d*(GLfloat)h/(GLfloat)w, 
				d*(GLfloat)h/(GLfloat)w, -d, d);
	else
		glOrtho(-d*(GLfloat)w/(GLfloat)h, d*(GLfloat)w/(GLfloat)h, 
				-d, d, -d, d);
	...
	glBegin(GL_POLYGON);
	for (GLdouble i = -5.0; i <= 5.0; i+=.1) {
		glVertex2d( i*1.0, heinFunc(i*1.0, g_N) );
	}

	for (GLdouble i = 5.0; i >= -5.0; i-=.001) {
		glVertex2d( i*1.0, -heinFunc(i*1.0, g_N) );
	}
	glEnd();
...
{
	if (m_bSuperEllipseEncoding) {
		__buildSuperEllipseTube(lineIdx);
	}
	else {
		__buildStreamtube(lineIdx);
	}
}
 ...
 */
void CSitubeRender::__buildSuperEllipseTube(unsigned long lineIdx)
{
	 ...
	 */
	GLint granularity = 2;
	GLint m_slod = m_lod*granularity*2;
	m_slod += 2;

	tube_vertices.resize ( szPts * m_slod * 3 );
	tube_normals.resize( szPts * m_slod * 3 );

	// szPts points contain szPts-1 line segments
	//tube_colors.resize( (szPts-1) * 3 );
	tube_colors.resize( szPts * m_slod * 3 );
	tube_faceIdxs.resize( (szPts-1) * m_slod * 4 );

	tube_encodedcolors.resize( szPts * m_slod * 4 );

	tube_ctxvertices.resize ( szPts * m_slod * 3 );
	tube_ctxcolors.resize ( szPts * m_slod * 4 );
	...
	for (unsigned long idx1 = 0, idx2 = 1; idx1 < szPts; idx1++) {
		...
		point_t sept, curpt;
		curpt.update(line[idx1*6+3], line[idx1*6+4], line[idx1*6+5]);
		...
		for (GLdouble i = -1.0*(m_lod/2); i <= 1.0*(m_lod/2); i+= (1.0/granularity)) {
			//sept.update (i*1.0, heinFunc(i*1.0, g_N), 0);
			//sept.update (i*1.0, heinFunc(i*1.0, (1.0-line[idx1*6+1])*g_N), 0);
			sept.update (i*1.0, heinFunc(i*1.0, 0.2+line[idx1*6+1]*g_N), 0);
			...
			tube_normals[ idx1*3 * m_slod + 3*l + 0 ] = nx = 
				//( (1 - cos(angle)) * rax * rax + cos(angle) ) * cos(theta) + ( (1 - cos(angle)) * rax * ray - sin(angle)*raz ) * sin(theta) ;
				( (1 - cos(angle)) * rax * rax + cos(angle) ) * sept.x + ( (1 - cos(angle)) * rax * ray - sin(angle)*raz ) * sept.y ;

			tube_normals[ idx1*3 * m_slod + 3*l + 1 ] = ny =
				//( (1 - cos(angle)) * rax * ray + sin(angle)*raz ) * cos(theta) + ( (1 - cos(angle)) * ray * ray + cos(angle) ) * sin(theta);
				( (1 - cos(angle)) * rax * ray + sin(angle)*raz ) * sept.x + ( (1 - cos(angle)) * ray * ray + cos(angle) ) * sept.y;

			tube_normals[ idx1*3 * m_slod + 3*l + 2 ] = nz =
				//( (1 - cos(angle)) * rax * raz - sin(angle)*ray ) * cos(theta) + ( (1 - cos(angle)) * ray * raz + sin(angle)*rax ) * sin(theta) ;
				( (1 - cos(angle)) * rax * raz - sin(angle)*ray ) * sept.x + ( (1 - cos(angle)) * ray * raz + sin(angle)*rax ) * sept.y ;

			normalize( tube_normals[ idx1*3 * m_slod + 3*l + 0 ], tube_normals[ idx1*3 * m_slod + 3*l + 1 ], tube_normals[ idx1*3 * m_slod + 3*l + 2 ] );

			tube_vertices[ idx1*3 * m_slod + 3*l + 0 ] = 0.05*nx + line[idx1*6+3];
			tube_vertices[ idx1*3 * m_slod + 3*l + 1 ] = 0.05*ny + line[idx1*6+4];
			tube_vertices[ idx1*3 * m_slod + 3*l + 2 ] = 0.05*nz + line[idx1*6+5];

			tube_colors [ idx1*3 * m_slod + 3*l + 0 ] = line [ idx1*6 + 0 ];
			tube_colors [ idx1*3 * m_slod + 3*l + 1 ] = line [ idx1*6 + 1 ];
			tube_colors [ idx1*3 * m_slod + 3*l + 2 ] = line [ idx1*6 + 2 ];
			tube_encodedcolors [ idx1*4 * m_slod + 4*l + 0 ] = 1.0;
			tube_encodedcolors [ idx1*4 * m_slod + 4*l + 1 ] = 1.0;
			tube_encodedcolors [ idx1*4 * m_slod + 4*l + 2 ] = 0.0;
			tube_encodedcolors [ idx1*4 * m_slod + 4*l + 3 ] = 1.0;

			tube_ctxvertices[ idx1*3 * m_slod + 3*l + 0 ] = 0.04*nx + line[idx1*6+3];
			tube_ctxvertices[ idx1*3 * m_slod + 3*l + 1 ] = 0.04*ny + line[idx1*6+4];
			tube_ctxvertices[ idx1*3 * m_slod + 3*l + 2 ] = 0.04*nz + line[idx1*6+5];
			tube_ctxcolors [ idx1*4 * m_slod + 4*l + 0 ] = r;
			tube_ctxcolors [ idx1*4 * m_slod + 4*l + 1 ] = g;
			tube_ctxcolors [ idx1*4 * m_slod + 4*l + 2 ] = b;
			tube_ctxcolors [ idx1*4 * m_slod + 4*l + 3 ] = (bKeyFiber?1.0:m_fAlpha);

			// find the maximal and minimal coordinats among the new
			// vertices of the streamtubes
			for (int j=0; j<3; ++j) {
				if ( tube_vertices[ idx1*3 * m_slod + 3*l + j ] > m_maxCoord[j] ) {
					m_maxCoord[j] = tube_vertices[ idx1*3 * m_slod + 3*l + j ];
				}
				if ( tube_vertices[ idx1*3 * m_slod + 3*l + j ] < m_minCoord[j] ) {
					m_minCoord[j] = tube_vertices[ idx1*3 * m_slod + 3*l + j ];
				}
			}
			...
			if ( szPts-1 > idx1 ) {
				tube_faceIdxs [ idx1*4 * m_slod + 4*l + 0 ] = idx1*m_slod + l;
				tube_faceIdxs [ idx1*4 * m_slod + 4*l + 1 ] = (idx1+1)*m_slod + l;
				tube_faceIdxs [ idx1*4 * m_slod + 4*l + 2 ] = (idx1+1)*m_slod + (l + 1)%m_slod;
				tube_faceIdxs [ idx1*4 * m_slod + 4*l + 3 ] = idx1*m_slod + (l + 1)%m_slod;
			}
 ...
 * created by loading the input geometry of streamlines
 */
void CSitubeRender::__buildStreamtube(unsigned long lineIdx)
{
	const vector<GLfloat> & line = m_loader.getElement( lineIdx );
	unsigned long szPts = static_cast<unsigned long>( line.size()/6 );
	GLfloat x1,x2,y1,y2,z1,z2,dx,dy,dz;
...
int CSitubeRender::mainstay() 
{
	...
	/* add the right-click menu as a special option panel */
	/*
	int szBtn = (int)m_buttons.size();
	for (int idx = 0; idx < szBtn; idx++) {
		addOptBtn(m_buttons[idx].text, "",
				m_buttons[idx].text2, "");
	}
	addOptBtn("#");
	*/
	...
	setClearColor( 102/255.0, 102/255.0, 102/255.0 );
...
void CSitubeRender::do_draw() 
{
	...
	glPolygonMode(GL_FRONT_AND_BACK, m_bWireframe?GL_FILL:GL_LINE);
...
void CSitubeRender::_realkeyResponse(unsigned char key, int x, int y)
{
	switch (key) {
		...
		case 's':
			{
				m_bSuperEllipseEncoding = !m_bSuperEllipseEncoding;
				_UPDATE_STREAMTUBES;
			}
			break;
		case '1':
		...
		case GLUT_KEY_HOME:
			{
				...
				CGLApp::onReshape(m_width, m_height);
