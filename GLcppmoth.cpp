	...
	m_bStereo(false)
{
	...
	m_clcolor[0] = .0, m_clcolor[1] = .0, m_clcolor[2] = .0;
	m_clcolor[3] = 1.0;
...
void CGLApp::draw()
{
	...
	*/
	...
	m_stereoinfo.eyesep =  fabs(mvmat[14]) * .5 / m_eyeZ;
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glFrustum(m_stereoinfo.left, m_stereoinfo.right, -m_stereoinfo.asp, m_stereoinfo.asp, m_near*8.0, m_far*2.0);
	...
	*/
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glFrustum(-m_stereoinfo.right, -m_stereoinfo.left, -m_stereoinfo.asp, m_stereoinfo.asp, m_near*8.0, m_far*2.0);
	//glFrustum(-m_stereoinfo.left-m_stereoinfo.asp, m_stereoinfo.right-m_stereoinfo.asp, m_stereoinfo.bottom, m_stereoinfo.top, m_near, m_far);
	//
...
{
	for (size_t i = 0; i < m_gadgets.size(); ++i) {
		m_gadgets[i]->setStereo( m_bStereo, 1.0, 40.0 );
		m_gadgets[i]->display();
	}
}
