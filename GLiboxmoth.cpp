...
void CGLIBoxApp::drawSpheres()
{
	...
	size_t szSphere = m_spheres.size();
	for (size_t idx = 0; idx < szSphere; idx++) {
		if ( m_spheres[idx].isRemovalBased() ) {
			m_spheres[idx].setFrameColor( 0.0, 1.0, 1.0 );
		}
		else {
			m_spheres[idx].setFrameColor( 1.0, 1.0, 1.0 );
		}
		m_spheres[idx].drawSphere(m_bBtnEnabled);
	}
	glPopMatrix();
	glPopAttrib();

	// draw the beacon, the box centric point projected to the ground
	if (m_bBeaconEnabled && m_curselsphereidx != -1) {
	}
}
...
int CGLIBoxApp::addSphere(const CIselsphere& selsphere)
{
	// from the programmability standpoint, any number of selection box is
	// acceptable but in actuality, too much, over 10 for instance, should be
	// mischievous and declined therefore
	if ( m_spheres.size() >= 10 ) {
		m_cout << "Too many selection boxes while Upper limit is 10.\n";
		return -1;
	}
	m_spheres.push_back( selsphere );
	// the responsiblity of setting the layout for the new selection box is
	// attributed to the user who creates the instance selbox
	return 0;
}

int CGLIBoxApp::addSphere()
{
	CIselsphere selsphere;
	if ( m_cout.isswitchon() ) {
		m_cout << "debug output switched on\n";
		selsphere.m_cout.switchoff(false);
		selsphere.m_cout.switchtime(true);
	}
	else {
		selsphere.m_cout.switchoff(true);
	}
	selsphere.associateObj(m_pvertices, &m_edgeflags, 
			m_dx, m_dy, m_dz);
	addSphere( selsphere );
	return _updateDefaultLayout();
}
...
void CGLIBoxApp::mouseResponse(int button, int state, int x, int y)
{
	...
	if ( m_spheres.size() >=1 ) {
		...
		case GLUT_DOWN: 
			{
				...
				size_t szSphere = m_spheres.size();
				for (size_t idx = 0; idx < szSphere; idx++) {
					curret = m_spheres[idx].onMouseClicked(button, state, x, y);
					if ( curret != 0 && curret != 2 && curret != 4) {
						continue;
					}
					if ( curret > maxret ) {
						m_curselsphereidx = idx;
						maxret = curret;
					}
				}
			}
		...
		case GLUT_UP:
			{
				if ( -1 != m_curselsphereidx ) {
					m_spheres[ m_curselsphereidx ].onMouseClicked( 
							button, state, 0,0);
					return;
				}
			}
...
{
	if ( -1 != m_curselsphereidx &&
		0 == m_spheres[ m_curselsphereidx ].onMouseMove( x, y ) ) {
		glutPostRedisplay();
		return;
	}
...
void CGLIBoxApp::mousePassiveMotionResponse(int x, int y)
{
	...
	m_curhlsphereidx = -1;
	for (size_t idx = 0; idx < szSphere; idx++) {
		if ( 0 == (m_spheres[idx].onMousePassiveMove(x, y) % 2))  {
			m_curhlsphereidx = idx;
...
void CGLIBoxApp::onReshape( GLsizei w, GLsizei h )
{
	...
	if (m_bFirstReshape) {
		if ( m_bIboxEnabled || m_spheres.size() >= 1) {
...
void CGLIBoxApp::do_draw ()
{
	...
	if ( m_spheres.size() < 1 && m_bRightmenuAvailable ) {
		drawRightmenuHint();
	}
...
int CGLIBoxApp::_updateDefaultLayout()
{
	...
	int szsphere = static_cast<int> (m_spheres.size());
	if ( 0 < szsphere ) {
		dx = w/szsphere;

		for (int idx = 0; idx < szsphere; ++ idx) {
			m_spheres[idx].setMinMax( 
					m_minx + idx*dx + tx, m_maxy/bszScale + ty, m_minz/bszScale + fabs(tz),
					//m_minx + (idx+1)*dx + tx, m_maxy + ty, m_maxz + tz);
					m_minx + (idx+1)*dx + tx, m_maxy/bszScale + ty/2, m_maxz/bszScale + fabs(tz));
			m_spheres[idx].setFrameColor(0.1, .1, 1.0);
		}
	}
