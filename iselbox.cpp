	...
	if (bBtnEnabled && m_pbuttons && -1 != m_highlightedFid) {
		...
		if (0 == _calcBtnLayout()) {
			for (size_t i = 0; i < szBtn; i++) {
				...
				if (/*"a"*/"Sel" == (*m_pbuttons)[i].text ||
					/*"e"*/"Rem" == (*m_pbuttons)[i].text ) {
					//(*m_pbuttons)[i].text = (m_bRemovalbased?"a":"e");
					(*m_pbuttons)[i].text = (m_bRemovalbased?"Sel":"Rem");
				}
				(*m_pbuttons)[i].draw();
			}
		}
...
{
	CIselbox* pother = new CIselbox(*this);
	...
	for (int i = 0; i <= numRow; ++i) {
		...
		for (int j = 0; j < numPerRow; ++j) {
			...
			for ( int k = 0; k < 3; ++k ) {
				...
				(*m_pbuttons)[ ibtn ].textpos [ pointidxs[k] ] = _coordlb[k] + 2;
			}
